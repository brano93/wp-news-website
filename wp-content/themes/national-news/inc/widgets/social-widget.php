<?php
if ( ! class_exists( 'National_News_Social_Widget' ) ) {
	/**
	 * Adds National News Social Widget.
	 */
	class National_News_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$national_news_social_widget = array(
				'classname'   => 'widget adore-widget social-widget style-1',
				'description' => __( 'Retrive Social Widget', 'national-news' ),
			);
			parent::__construct(
				'national_news_social_widget',
				__( 'Adore Widget: Social Widget', 'national-news' ),
				$national_news_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Link %d :', 'national-news' ), $i ); ?></label>
